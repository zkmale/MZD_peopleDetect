...
#elif defined(_AIX)
	struct __pthrdsinfo data;
	int reg_size = 0;
	pthread_t pt = pthread_self();
	int retval = pthread_getthrds_np(&pt, PTHRDSINFO_QUERY_TID, &data, sizeof(data), NULL, &reg_size);
	int tid = (!retval) ? data.__pi_tid : 0;
...
{
	memory_buf_t data;
	wstr_to_utf8buf(filename, data);
	return SPDLOG_BUF_TO_STRING(data);
}
#else
...
#    else
	size_t len = 0;
	char data[128];
	bool ok = ::getenv_s(&len, data, sizeof(data), field) == 0;
