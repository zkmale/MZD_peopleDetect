...
#ifdef _MSC_VER
FMT_FUNC auto fmt_snprintf(char* data, size_t size, const char* fmt, ...)
	-> int {
  auto args = va_list();
  va_start(args, fmt);
  int result = vsnprintf_s(data, size, _TRUNCATE, fmt, args);
  va_end(args);
  return result;
}
#endif
...
struct formatter<detail::bigint> {
	...
	typename FormatContext::iterator {
		...
		if (n.exp_ > 0)
			out = format_to(out, FMT_STRING("p_data_start{}"),
