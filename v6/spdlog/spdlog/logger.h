...
public:
	...
	void log(log_clock::time_point log_time, source_loc loc, level::level_enum lvl, wstring_view_t msg)
	{
		...
		memory_buf_t data;
		details::os::wstr_to_utf8buf(wstring_view_t(msg.data(), msg.size()), data);
		details::log_msg log_msg(log_time, loc, name_, lvl, string_view_t(data.data(), data.size()));
		log_it_(log_msg, log_enabled, traceback_enabled);
	}
	...
	void log(source_loc loc, level::level_enum lvl, wstring_view_t msg)
	{
		...
		memory_buf_t data;
		details::os::wstr_to_utf8buf(wstring_view_t(msg.data(), msg.size()), data);
		details::log_msg log_msg(loc, name_, lvl, string_view_t(data.data(), data.size()));
		log_it_(log_msg, log_enabled, traceback_enabled);
	}
...
protected:
	...
	void log_(source_loc loc, level::level_enum lvl, string_view_t fmt, Args &&... args) {
		...
		SPDLOG_TRY {
			...
			fmt_lib::vformat_to(std::back_inserter(data), fmt, fmt_lib::make_format_args(std::forward<Args>(args)...));
#else
...
#    endif
			...
			memory_buf_t data;
			details::os::wstr_to_utf8buf(wstring_view_t(wbuf.data(), wbuf.size()), data);
			details::log_msg log_msg(loc, name_, lvl, string_view_t(data.data(), data.size()));
			log_it_(log_msg, log_enabled, traceback_enabled);
		}
		SPDLOG_LOGGER_CATCH(loc)
	...
	void log_(source_loc loc, level::level_enum lvl, const T &msg)
	{
		...
		{
			memory_buf_t data;
			details::os::wstr_to_utf8buf(msg, data);
			details::log_msg log_msg(loc, name_, lvl, string_view_t(data.data(), data.size()));
			log_it_(log_msg, log_enabled, traceback_enabled);
		}
		SPDLOG_LOGGER_CATCH(loc)
