...
bool function_GetAngle(float &x, float &y) {
    ...
    //static auto client = httplib::Client("192.168.8.115", 18080);
        ...
        if (!frame_buffer.empty()) {
            ...
            if (!frame1.empty()){
                ...
                if(k%angle_interval == 10){
                    ...
                    function_GetAngle(angle, angle_y);   //angle显示的为负数

                    std::cout << "x_angle = " << angle << std::endl;
                    //std::cout << "y_angle = " << angle_y << std::endl;
