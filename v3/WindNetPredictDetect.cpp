...
void WindNetDetect::send_json_people(cv::Mat img, std::string label, std::string level){
    ...
    auto result = client.Post("/update", imgInfo.to_json_people(), "application/json");
#endif
...
std::vector<int> WindNetDetect::readFileJson(){
    Json::Reader reader;
	Json::Value root;
    Json::Value root1;
    
    std::ifstream in("./rtsp_sign_result.json", std::ios::binary);
    ...
    if (reader.parse(in, root))
	{
		...
		for (int i = 0; i < lens; i++) {
			root1 = root[i];
			if (root1["sign_type"] == "DangerZone") {
				int start_pot_x = root1["sign_data"]["start_pot"]["x"].asInt();
				int start_pot_y = root1["sign_data"]["start_pot"]["y"].asInt();
				int end_pot_x = root1["sign_data"]["end_pot"]["x"].asInt();
				int end_pot_y = root1["sign_data"]["end_pot"]["y"].asInt();
                point_2.push_back(start_pot_x);
                point_2.push_back(start_pot_y);
                point_2.push_back(end_pot_x);
                point_2.push_back(end_pot_y);

				std::cout << "start_pot_x =  " << point_2[0]  << std::endl;
				std::cout << "start_pot_y =  " << point_2[1] << std::endl;
				std::cout << "end_pot_x =  " << point_2[2] << std::endl;
				std::cout << "end_pot_y =  " << point_2[3] << std::endl;
			}
		}
	}
	else
