...
void WindNetDetect::send_json_people(cv::Mat img, std::vector<std::string> label, std::string level){
    std::string out = Mat2Base64(img,".jpg");
    //std::cout << out << std::endl;
...
std::vector<int> WindNetDetect::readFileJson(){
	...
	{
		//cv::Rect rect;
		for (unsigned int i = 0; i < root["numericalArray"].size(); i++)
		{
			//std::string ach = root["numericalArray"][i].asString();
			int ach = root["numericalArray"][i].asInt();
			point_2.push_back(ach);
